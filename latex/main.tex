...
Se elaboró un mapa mental como técnica inicial para recolectar y organizar la información relativa al contexto del negocio, actores involucrados, problemática, requerimientos, los objetivos y las salidas esperadas del proyecto. Esta herramienta posibilita tener una visión completa del entorno y establecer trazabilidad entre los elementos empresariales y los objetivos del componente. La Figura \ref{fig:mind_map_lev_inf} presenta el mapa mental realizado, el cual contiene siete apartados:

\begin{itemize}
    \item{Problemática:} Identifica las restricciones presentes en la gestión de la demanda de los clientes no regulados, asociadas a la alta variablidad en sus patrones de consumo y métodos tradicionales de análisis insuficientes.
    ...
    \item{Objetivo del levantamiento:} Establecer criterios preliminares para una segmentación útil, que apoye la planificación energética fundamentada en evidencias y que facilite la toma de decisiones.
    \item{Actores involucrados:} Se identifica a la EEQ como la entidad responsable de proveer de manera confiable y eficaz la información relativa a los clientes no regulados como los sujetos de estudio, y al área de Planificación como principal usuario de los resultados.
    \item{Información inicial:} Se describe las fuentes de datos existentes, que son los registros de consumo mensual de 388 clientes a lo largo del año 2023, con intervalos de medición establecidos entre 5 y 15 minutos, así como variables relacionadas con la energía y la potencia.
    \item{Salidas esperadas:} Se determinan los productos que resultarán del análisis: la curva característica de cada cliente, la curva del día con máxima demanda, archivos separados por coma y de texto plano, además de las agrupaciones con los clientes segmentados en base a la similitud de sus curvas de carga características.
\end{itemize}
...
\begin{itemize}
    \item{Obtener una visión completa de los patrones de consumo asociados a los clientes no regulados del sector eléctrico.}
    \item{Generar agrupaciones que posibiliten el reconocimiento de comportamientos energéticos generalizados dentro del grupo de clientes.}
    \item{A partir de la información obtenida, crear estrategias diferenciadas para gestionar la demanda y planificar la red del sistema eléctrico.}
\end{itemize}

Se considera que el proyecto fue exitoso si las agrupaciones resultantes permiten identificar patrones energéticos característicos y generen información interpretable para la EEQ, orientada al cumplimiento de los objetivos establecidos.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item{Entorno de desarrollo de Python (con librerías relacionadas a ciencia de datos), orquestación del proceso ETL utilizando Apache Airflow sobre Docker y MongoDB Atlas como base de datos en nube para guardar los registros correspondientes a las curvas de carga características}
        \item{Personal del área de Planificación de la EEQ como apoyo técnico y proveedor de la información requerida.}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item{Para el análisis serán considerados únicamente aquellos registros correspondientes a días laborables, todos los demás días (fines de semana y feriados) serán excluídos.}
        \item{Garantizar que los datos tengan la consistencia característica de una serie temporal (independientemente del intervalo)}
    ...
    \item{Supuestos:}
        ...
        \item{Las mediciones recopiladas reflejan de manera precisa el comportamiento energético real de los clientes.}
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item{Restricciones de capacidad computacional debido al gran volúmen de datos que se requiere manipular} 
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item{Evaluar la calidad de los clústeres utilizando métricas que cuantifican la calidad de los mismos (Silhouette Score, Davies-Bouldin Index, Calinski-Harabasz Index y correlación.)}
\end{itemize}
...
\begin{enumerate}
    ...
    \item{Entendimiento de los datos:} Análisis exploratorio de los datos, validación de la calidad, consistencia y completitud de los mismos.
    ...
    \item{Evaluación:} Comparación de las agrupaciones generadas por cada algoritmo, haciendo uso de métricas que cuantifican la calidad de las mismas.
    \item{Adaptación de CRISP-DM en la fase final de despliegue:} La etapa de despliegue será sustituida por un análisis comparativo entre los resultados obtenidos por cada algoritmo, utilizando gráficos y técnicas de representación de clústeres, con el fin de generar conocimiento de utilidad para el área de Planificación de la EEQ.
\end{enumerate}
...
Tras la recolección de datos, se llevó a cabo una revisión inicial de los archivos para determinar su estructura general y las variables que contienen. Esta tarea permitió determinar la naturaleza de los datos y evaluar el tipo de formato que poseen. A pesar de venir en archivos separados por coma (.csv), los archivos no necesariamente están separados por coma, algunos vienen separados por tabulaciones y otros por punto y coma, incluyen datos sobre el consumo y la temporalidad de los mismos. Se pudo notar lo siguiente:
...
En la Figura \ref{fig:fig_desc_datos} se puede apreciar todo lo mencionado anteriormente.
...
\begin{itemize}
    \item Gráfico de barras: Cantidad de clientes según el tipo de medición que poseen sus archivos (potencias o energía).
...
Se llevaron a cabo varias comprobaciones con el objetivo de cuantificar y observar el impacto generado de estas anomalías sobre el conjunto total de datos. Para ello, se elaboró una imagen que resume los resultados obtenidos en términos de uniformidad de formato, consistencia numérica y presencia de valores nulos y ausentes.
...
Toda esta arquitectura se encuentra definida en \texttt{docker-compose.yaml}, archivo que fue modificado para incluir de manera adicional tres volúmenes compartidos entre el sistema de archivos local y el entorno aislado de Apache Airflow:

\begin{itemize}
    \item /data: Contiene todos los archivos con formato \texttt{.csv} correspondientes a las mediciones mensuales de todos los clientes no regulados.
    \item /outputs: Este volúmen fue creado con la finalidad de almacenar los entregables adicionales requeridos por la EEQ.
    \item /utils: Contiene un archivo con variables de entorno (\texttt{.env}), cuyo contenido son credenciales para comunicarse con MongoDB Atlas. Adicionalmente contiene el archivo \texttt{utilities.py}, el cual contiene funciones auxiliares para comunicación con la base de datos, estandarización de formato de fechas, generación de curvas, entre otros.
\end{itemize}

La Figura \ref{fig:volumenes_dockercompose} ilustra la modificación realizada al archivo \texttt{docker-compose.yaml} para añadir los volúmenes compartidos mencionados anteriormente:
...
Una vez dentro de la CLI de cada servicio, se hizo uso del comando \texttt{python -m pip install pandas numpy scikit-learn pymongo dotenv matplotlib} para instalar las librerías requeridas por Python para el correcto funcionamiento del DAG. La Figura \ref{fig:instalacion_librerias_worker} muestra todo el proceso mencionado anteriormente:
...
En la Figura \ref{fig:etl_definition} se describen las operaciones concretas que cada tarea realiza durante las fases de extracción, transformación y carga de datos, así como las tareas que integran el DAG. Este desglose permite especificar el alcance de cada tarea sobre el conjunto de datos y así brindar un flujo organizado para la elaboración detallada de los procesos de selección, limpieza, construcción, integración y formateo de los datos que se describen en las siguientes secciones.

\subsubsection*{Seleccionar los datos}
En esta tarea, se hizo la selección del conjunto de datos que serán utilizados en las etapas siguientes del análisis, garantizando que coincidieran con las metas establecidas en la fase de entendimiento del negocio. Los datos se obtuvieron a partir de los registros históricos de mediciones mensuales de los clientes no regulados de la EEQ, correspondientes a 2023.\\
...
\begin{enumerate}
    ...
    \item Exclusión de días no laborables y feriados: Los registros correspondientes a fines de semana y feriados nacionales del año 2023 fueron eliminados. Esta exclusión se basó en que los clientes no regulados tienen patrones de consumo altamente asociados a su actividad laboral. En consecuencia, estos días no resultan útiles para el análisis.
    \item Separador de miles: En tareas anteriores se han detectado valores que poseen la coma como separador de miles y punto como separador decimal, esto se atribuye únicamente al grupo de clientes con mediciones de energía. Se eliminaron las comas y se estableció el tipo de dato como flotante para garantizar que Python pueda procesarlos correctamente.
\end{enumerate}
...
Con este propósito, se implementaron las transformaciones correspondientes en las tareas \texttt{transformar\_datos\_grupo01} y \texttt{transformar\_datos\_grupo02} que forman parte del DAG \texttt{etl\_dag\_datos\_consumo}, donde se crearon las variables potencia aparente y potencia aparente escalada.\\
...
\begin{equation}
    S = \sqrt{P^{2} + Q^{2}}\label{eq:pitagoras_pa}
\end{equation}
...
\begin{equation}
    S = E_{15\text{min}} \times 4\label{eq:energia_pa}
\end{equation}

Una vez calculada la potencia aparente para los dos grupos de clientes haciendo uso de las ecuaciones \eqref{eq:pitagoras_pa} y \eqref{eq:energia_pa}, se procede a normalizar con el fin de garantizar la comparabilidad entre curvas de carga con diferentes magnitudes de consumo, para esto se utilizó la técnica de escalado mínimo-máximo, la cual transforma los valores de potencia aparente dentro de un rango definido, que en este caso es [0, 1].\\

Se decidió implementar un escalado individual para cada día de cada cliente, ya que existen días en los que la demanda es mucho más alta que en otros. Con este método se garantiza una normalización balanceada entre los diferentes días, evitando que los días con valores altos 'aplanen' al resto de registros. El procedimiento consiste en escalar cada medición con respecto a los valores mínimo y máximo correspondientes a su propio día, siguiendo la ecuación \eqref{eq:normalizacion}:
\begin{equation}
    S_{\text{escalada}} = \frac{S - S_{\text{min}}}{S_{\text{max}} - S_{\text{min}}}\label{eq:normalizacion}
\end{equation}
...
La elección de escalar cada día de manera individual fue el resultado de llevar a cabo un estudio comparativo entre la normalización global aplicada al año completo y la normalización diaria que se empleó. La Figura \ref{fig:comparacion_normalizacion} evidencia que la normalización global tiende a suavizar la curva característica debido a que hay días con valores significativamente más altos respecto al resto. El comportamiento mencionado anteriormente es particularmente común en clientes no regulados, cuyos patrones de consumo presentan una alta variabilidad al estar vinculados con su actividad económica. En contraste, la normalización diaria mantiene la estructura relativa de cada día, evitando así que un día con valores elevados respecto al resto ``aplane'' la curva, dando como resultado una representación más precisa del patrón de consumo característico asociado al cliente.
...
Una vez concluido el de transformación individual de cada grupo de clientes (selección, limpieza y construcción), se procedió a unificar los datos procesados provenientes de ambos grupos, con el objetivo de consolidar un único conjunto de datos global y proceder con el cálculo de las curvas características representativas anuales. La tarea \texttt{transformar\_datos\_unificados} fue la encargada de realizar dicha integración, la cual forma parte del DAG \texttt{etl\_dag\_datos\_consumo}. En dicha tarea se combinaron los diccionarios generados en los flujos de transformación individuales de cada grupo en un único diccionario consolidado.\\

Una vez consolidada toda la información, se procedió a obtener la curva tipo, con ayuda de la función \texttt{obtener\_coords\_curva\_tipo} se realizó una agregación por hora, calculando la mediana de la potencia aparente escalada a lo largo de todos los días del año. Se hizo uso de la mediana debido a que es una medida robusta frente a valores atípicos, los cuales son propios e inherentes al comportamiento energético de los clientes no regulados.\\

Para concluir, se integraron las curvas tipo de todos los clientes en un solo DataFrame consolidado, em el cual cada fila corresponde a un cliente y cada columna a una marca temporal única, desde las 00:00 hastas las 23:45 en intervalos de 15 minutos.

\subsubsection*{Formatear los datos}
Para concluir la fase de preparación de los datos, se realizaron las modificaciones finales al formato del conjunto de curvas tipo generado en la tarea anterior, con el objetivo de preparar la información para su posterior almacenamiento y modelado. Estas operaciones se ejecutaron en la tarea \texttt{cargar\_datos\_curvas\_tipo}, la cual es parte del DAG \texttt{etl\_dag\_datos\_consumo}. Las actividades realizadas fueron:

\begin{enumerate}
    \item Se comprobó que cada registro del DataFrame contara un identificador exclusivo para el cliente, convirtiendo el campo 'Cliente' a tipo cadena de caracteres, debido a que dicho identificador puede ser alfanumérico.
    \item Se tranformó el DataFrame consolidado de las curvas tipo en un diccionario orientado a registros, lo que permitió insertar los documentos directamente en la base de datos de MongoDB Atlas.
\end{enumerate}

Finalmente, se creó un índice sobre el campo ``Cliente'' con el fin de optimizar consultas dentro de la base de datos y se procedió a cargar los registros en la colección \texttt{CurvasTipoAnuales}, dando por finalizada la fase de preparación de los datos y obteniendo así un conjunto de datos completamente limpio, estructurado, formateado y preparado para su uso en la fase de modelado. 

\subsection{Modelado}
La fase de modelado es el núcleo de CRISP-DM, en la que se crean los modelos que serán aplicados sobre el conjunto de datos ya preparado. En esta fase, la comprensión de los datos y del negocio se traduce en un grupo de algoritmos que pueden detectar patrones de consumo a partir de las curvas de carga características generadas en la fase anterior. Siguiendo la guía de CRISP-DM, es necesario escoger la técnica de modelado a utilizar y definir previamente los criterios mediante los cuales se evaluaran los resultados, antes de implementar cualquier modelo. Posteriormente se desarrollan los modelos y se analizan sus resultados de acuerdo con los criteros técnicos y del negocio.
...
La Figura \ref{fig:proceso_agrupacion} presenta un diagrama que sintetiza las diversas tareas que intervienen en el proceso de agrupación que se va a desarrollar en el presente componente. Este diagrama fue creado en base al cuaderno de trabajo utilizado para realizar clustering, se ilustra la secuencia de trabajo que se implementó: obtener las curvas desde la base de datos, ajustar las curvas para comparabilidad, determinar el número óptimo de agrupaciones, optimizar parámetros de los algoritmos, aplicar los algoritmos, generar agrupaciones, evaluar las agrupaciones (tanto cuantitativa como cualitativamente) y seleccionar el algoritmo con el mejor desempeño.
...
Dado que el objetivo del análisis es encontrar similitudes en la estructura de las curvas de carga, el diseño de prueba se enfoca en evaluar la consistencia interna y la separación entre los clústeres mediante métricas internas de validación. En la tabla \ref{tab:metricas_definicion} se presenta la selección y justificación de las métricas que se van a emplear, conforme a la bibliografía especializada en la agrupación de perfiles de carga eléctrica.
...
Silhouette Score &
Analiza la calidad de la asignación de cada curva de carga a su clúster respectivo, teniendo en cuenta al mismo tiempo el agrupamiento interno del clúster y la distancia con respecto a los clústeres adyacentes. Los valores oscilan entre -1 y 1, siendo los que están más cerca de 1 un indicativo de una mejor asignación. En el contexto de las curvas de carga eléctrica, esta métrica posibilita examinar la separación global entre agrupaciones, no obstante, puede mostrar valores moderados como resultado de la superposición natural de los patrones de consumo \cite{Chicco2012OverviewClustering}. \\
\hline

Índice Davies--Bouldin (DBI) &
Cuantifica la relación entre la distancia de los clústeres y su dispersión interna, penalizando a los agrupamientos con clústeres poco compactos o que están muy próximos. Valores más bajos del índice señalan una mejor calidad de agrupamiento. Este índice se ha empleado en investigaciones de segmentación de datos eléctricos y series temporales, incluso en aplicaciones concretas sobre perfiles de carga, gracias a su aptitud para analizar simultáneamente la separación y compacidad \cite{li2023shapebased,halkidi2001clusteringvalidation}. \\
\hline
...
\begin{enumerate}
    ...
    La selección de los parámetros óptimos para cada algoritmo se realizó utilizando métricas internas de validación, priorizando aquellas configuraciones que maximizaran la homogeneidad interna de los clústeres. De esta manera, se garantizó que cada algoritmo fuera evaluado bajo condiciones ajustadas a las características de las curvas de carga analizadas.
...
Los resultados cuantitativos y el análisis comparativo de los algoritmos evaluados se presentan en la sección \ref{sec:resultados} ``Resultados''  del presente documento.
...
La evaluación de los resultados es realizada mediante un análisis cuantitativo y cualitativo de las agrupaciones obtenidas. Dicho análisis se presenta en la sección \ref{sec:resultados} ``Resultados'', donde se analizan las distintas métricas de validación, las visualizaciones de los clústeres y la interpretación energética de los patrones encontrados.


\subsubsection*{Revisar el proceso}
En esta tarea se realizó una revisión integral del proceso seguido a lo largo del desarrollo del presnete componente bajo la metodología CRISP-DM, abarcando desde la comprensión del negocio hasta la evaluación de los modelos de clustering.\\

Se verificó que las fases de preparación de los datos, modelado y evaluación se ejecutaron de manera coherente en función de los objetivos planteados, y que las decisiones adoptadas en cada etapa se encuentran debidamente justificadas. En particular, se constató que el uso de las curvas tipo como representación del consumo anual, la normalización por día, el ajuste realizado a las curvas, la hiperparametrización de los algoritmos y la elección de métricas de evaluación orientadas a la forma de las curvas fueron adecuadas para el problema abordado.\\
...
Uno de los principales aprendizajes del proyecto es la importancia de disponer de un proceso ETL sólido y adecuadamente diseñado, lo cual cobra especial relevancia al trabajar con datos reales de consumo que se caracterizan por tener irregularidades de formato, valores ausentes y comportamientos atípicos propios de los clientes no regulados. En este contexto, la preparación de las curvas de carga fue un punto clave para poder garantizar la calidad y robustez de los agrupamientos obtenidos.\\
...
En este apartado se presentan y analizan los resultados obtenidos a partir de la aplicación y evaluación de los algoritmos de clustering sobre las curvas de carga características anuales de los clientes no regulados. En consecuencia, los resultados consolidados son producto de lo desarrollado en las fases de Modelado y Evaluación de la metodología CRISP-DM, y permiten la comparación del rendimiento alcanzado por los algoritmos que se han implementado, medir la calidad de los agrupamientos generados y determinar el modelo o método de clustering más adecuado para la identificación de patrones de consumo energético.
...
La correlación intra-cluster ha sido la métrica principal usada. Es el valor promedio de la correlación entre cada curva de carga individual y la curva media de cada clúster. Esta métrica permite calcular directamente la semejanza entre la forma de las curvas agrupadas, aspecto fundamental en el presente trabajo, dado que el objetivo es agrupar a los clientes no regulados en función del comportamiento energético que se refleja en la forma de su curva de carga característica anual.\\

La correlación intra-clúster es una medida eficaz para el estudio de perfiles de carga eléctrica en el sentido de que se centra en la semejanza estructural de las curvas y no en el valor absoluto de sus magnitudes, lo que permite que la comparación de los patrones de consumo se lleve a cabo sin necesidad de considerar la magnitud de la demanda individual que presenta cada uno de los clientes \cite{labeeuw2013residential,Chicco2012OverviewClustering}. Este planteamiento resulta especialmente oportuno en el caso de las curvas normalizadas, donde el objetivo principal se centra en la forma del perfil y no en su escala.\\

Los valores de correlación intra-clúster promedio de los algoritmos empleados evidencian las variaciones existentes en el grado de homogeneidad de los grupos generados, constituyendo así un criterio directo para evaluar la calidad de los agrupamientos obtenidos
...
En la Tabla \ref{tab:metricas_clustering} se muestra la comparación numérica del rendimiento de los algoritmos de clustering analizados, de manera independiente, a partir de los valores obtenidos con las métricas de evaluación complementarias, en contraste con la correlación intra-clúster promedio, definida previamente como la métrica principal para el presente análisis.
...
El análisis de resultados evidencia que los valores obtenidos para el Silhouette Score son relativamente bajos para todos los algoritmos analizados. Este comportamiento se atribuye a las características naturales de las curvas de carga, que en el caso propuesto tienen una elevada variabilidad temporal y densidades heterogéneas. De acuerdo con lo expuesto en el trabajo realizado en \cite{jain2021validating}, este fenómeno es frecuente en conjuntos de datos reales de demanda eléctrica, debido a que dicho índice pierde robustez en escenarios con agrupaciones densas y patrones con alta dispersión interna. \\

En este sentido, los resultados obtenidos apuntan a interpretar el Silhouette Score como un criterio complementario y no como un indicador determinante que deba adoptarse de forma aislada, relegando a un segundo plano métricas relacionadas directamente con la semejanza estructural de las curvas de carga. Desde esta perspectiva, la correlación intra-cluster resulta mucho más representativa de lo que se persigue en la presente investigación, esto es, evaluar de forma explícita la similitud en la forma de los perfiles de consumo energético.\\

Según los resultados obtenidos, se puede observar que el algoritmo Spectral Clustering es el que alcanza el mayor valor de correlación intra-cluster promedio entre los métodos de clustering analizados. Al tratarse de una métrica directamente vinculada con el criterio de similitud de la forma de la curva de carga, se la considera como el principal indicador de la calidad del agrupamiento. Asimismo, su comportamiento en las métricas complementarias es comparable al del resto de los algoritmos evaluados, sin evidenciar diferencias que desaconsejen su utilización.

\subsection{Evaluación cualitativa y análisis visual de los clústeres}
La evaluación de los resultados permite establecer en qué medida los modelos de clustering construidos logran alcanzar el objetivo principal del análisis; este consiste en segmentar los clientes no regulados en función de la forma de su curva característica anual de consumo energético.\\

A diferencia de los problemas de aprendizaje supervisado, donde las etiquetas de referencia permiten evaluar de forma directa el rendimiento, en el caso del clustering la evaluación debe abordarse de manera multidimensional. En este trabajo, dicha evaluación se fundamenta en tres ejes complementarios:
\begin{itemize}
...
Los resultados de agrupación obtenidos a partir de la ejecución de los algoritmos K-Means, Gaussian Mixture, Birch y Spectral Clustering dieron lugar a agrupaciones consistentes en todos los casos, obteniéndose curvas tipo claramente diferenciables para cada clúster.\\

Este aspecto es relevante, ya que indica que los algoritmos han sido capaces de captar similitudes reales en la forma de las curvas de carga de los clientes, y no simplemente diferencias asociadas al consumo absoluto. Este comportamiento guarda una relación directa con las decisiones adoptadas en las fases de preparación y modelado de los datos, donde las curvas fueron normalizadas y ajustadas, de modo que se eliminaran sesgos asociados a la escala y a desplazamientos verticales.\\

Al contemplar simultáneamente las curvas tipo promedio generadas por las diferentes técnicas, se puede observar que, con independencia del método empleado, los patrones de consumo se identifican de forma consistente. Este comportamiento se manifiesta en perfiles con un comportamiento horario estable a lo largo del día, perfiles que presentan picos claramente definidos en intervalos horarios específicos y perfiles con alta variabilidad a lo largo de la jornada.\\

La Figura \ref{fig:curvas_tipo_algoritmos_2x2} muestra de manera visual una comparación entre las curvas tipo promedio generadas para cada clúster, según el algoritmo empleado para su obtención:
