...
{\bfseries\fontsize{12}{14}\selectfont DMQ, enero 2026\par}
...
\chapter*{\makebox[\textwidth][c]{CERTIFICACIONES}}
...
\textbf{JOSAFÁ DE JESÚS AGUIAR PONTES}\\
\textbf{DIRECTOR}
\end{center}
...
\chapter*{\makebox[\textwidth][c]{DECLARACIÓN DE AUTORÍA}}
...
\textbf{ANDRÉS ANTONIO ZAMBRANO ALQUINGA}\\ 
\textbf{JOSAFÁ DE JESÚS AGUIAR PONTES}\\
\textbf{BORIS ALFONSO ASTUDILLO ESPINOZA}
...
\chapter*{\makebox[\textwidth][c]{DEDICATORIA}}
...
\chapter*{\makebox[\textwidth][c]{AGRADECIMIENTOS}}
...
\chapter*{\makebox[\textwidth][c]{ÍNDICE DE CONTENIDO}}
\addcontentsline{toc}{chapter}{ÍNDICE DE CONTENIDO}

\begingroup
\renewcommand{\chapter}[2]{} % anula el \chapter* interno que usa \tableofcontents
\tableofcontents
\endgroup
...
\chapter*{\makebox[\textwidth][c]{RESUMEN}}
...
\chapter*{\makebox[\textwidth][c]{ABSTRACT}}
...
\bfseries\fontsize{17pt}{16pt}\selectfont DESCRIPCIÓN DEL COMPONENTE DESARROLLADO}
...
\hline
\multicolumn{1}{|c|}{\textbf{Herramienta}} &
\multicolumn{1}{c|}{\textbf{Descripción de la herramienta}} \\
\hline
\endfirsthead
...
Se elaboró un mapa mental como técnica inicial para ordenar, sintetizar y estructurar la información recopilada anteriormente. Esta herramienta posibilita tener una visión completa del entorno y establecer trazabilidad entre los elementos empresariales y los objetivos del componente. La Figura \ref{fig:mind_map_lev_inf} presenta el mapa mental realizado, agrupado en siete apartados:
\begin{itemize}
...
\begin{adjustwidth}{-2cm}{-2cm}
    \centering
    \includegraphics[scale=0.34]{./imgs/Comprensión del Negocio - Levantamiento de Información2.png}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item{Entorno de desarrollo de Python, orquestación del proceso ETL utilizando Apache Airflow sobre Docker y MongoDB Atlas como base de datos en nube para guardar los registros correspondientes a las curvas de carga características}
        \item{Personal del área de Planificación de la EEQ como apoyo técnico y proveedor de la información requerida.}
    \end{itemize}
    \item{Requerimientos:}
    \begin{itemize}
        \item{Para el análisis serán considerados únicamente aquellos registros correspondientes a días laborables, los demás días serán excluídos.}
        \item{Se debe garantizar que los datos tengan la consistencia característica de una serie temporal (independientemente del intervalo)}
        \item{Se tiene que restringir el alcance a la creación y evaluación de los modelos de agrupamiento, sin contemplar un despliegue en producción.}
    \end{itemize}
    \item{Supuestos:}
        \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item{El análisis se realizó exclusivamente con los datos del año 2023, debido a que este período es el que mayor nivel de completitud y consistencia presenta.} 
    \end{itemize}
\end{itemize}
...
Se desarrolló un plan de ejecución, el cual fue estructurado en base a la metodología CRISP-DM y ajustado a las necesidades del presente componente, basándose en la información levantada y los objetivos establecidos previamente. La Figura \ref{fig:plan_proyecto} muestra el flujo definido para guiar el desarrollo del proyecto.

\begin{figure}[ht]
\begin{center}
\includegraphics[scale=0.26]{./imgs/PlanProyecto.png}
\caption{{Flujo establecido para el plan de ejecución del proyecto}}
\label{fig:plan_proyecto}
\end{center}
\end{figure}

\subsection*{\underline{Entendimiento de los datos}}
...
\begin{itemize}
    ...
    \item Todos los archivos poseen una cabecera con información del cliente u otros aspectos relevantes.
    \item Existen dos grupos de clientes: los que tienen únicamente mediciones de energía (b) y los que tienen mediciones de potencia activa y reactiva (a).
    \item El formato de la fecha varía (ej: AAAA/MM/DD, AAAA-MM-DD, DD/MM/AAAA)
    \item Algunos archivos presentan ausencia de registros debido a inconsistencias o pérdidas derivadas del sistema de medición (a).
    \item Algunos archivos poseen líneas de resumen, las cuales incluyen totales ponderados agregados para intervalos específicos (b).
    \item Algunos valores dentro de los registros contienen coma como separador de miles y punto como separador decimal, por ejemplo: 1,203.239 (b).
\end{itemize}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[scale=0.483]{./imgs/desc_datos2.png}
...
Como se puede apreciar en el primer gráfico (superior izquierdo), el formato más común es el YYYY-MM-DD (66,3\%), seguido por YYYY/MM/DD (21,5\%) y DD/MM/YYYY (12,2\%). Esta falta de uniformidad en el formato de fecha es crítico debido a que para construir series temporales se debe tener un formato único, se requerirá un proceso de estandarización de formato de fecha para solucionarlo.\\
...
Por último, el cuarto gráfico (inferior derecho) indica que dentro del grupo de clientes que solo tienen mediciones de potencias, existen en total 17 registros nulos, mientras que en el grupo de clientes que posee únicamente mediciones de energía, existe un total de 2259 registros nulos. El volúmen de registros que poseen valores nulos es mínimo respecto al total de registros global y por cliente, por lo que se considera apropiado utilizar un método de interpolación para llenar dichos valores. Este método posibilitará que sean estimados en función de la tendencia particular de cada cliente, conservando la coherencia temporal y el comportamiento original de los datos.

\subsection*{\underline{Preparación de los datos}}
En esta etapa, se implementó un proceso ETL a través de Apache Airflow en un entorno Docker, tal y como se ilustra en la Figura \ref{fig:proceso_etl}, gracias al cual se pudo orquestar de manera eficiente el flujo de procesamiento de los datos de consumo, garantizando automatización y trazabilidad.
...
La Figura \ref{fig:volumenes_dockercompose} ilustra la modificación realizada al archivo \texttt{docker-compose.yaml} para añadir los volúmenes compartidos mencionados anteriormente. El contenido completo de dicho archivo se encuentra disponible en el Anexo \ref{anexo:docker_compose}.
...
Con el propósito de complementar la vista general de la Figura \ref{fig:dag_etl_process}, en la Figura \ref{fig:etl_definition} se describen las operaciones concretas que cada tarea realiza durante las fases del proceso ETL, así como las tareas que integran el DAG. Este desglose permite especificar el alcance de cada tarea sobre el conjunto de datos y así brindar un flujo organizado para la elaboración detallada de los procesos de selección, limpieza, construcción, integración y formateo de los datos que se describen en las siguientes secciones.
...
\begin{enumerate}
    \item Estandarización de formatos de fecha: Todos los formatos de fecha detectados fueron estandarizados en uno solo (YYYY/MM/DD).
    \item Tratamiento de valores nulos: Se empleó un interpolador spline cúbico para llenar los valores faltantes identificados en tareas anteriores.
    \item Exclusión de días no laborables y feriados
    \item Eliminación de separadores de miles
\end{enumerate}
...
\begin{figure}[ht]
\begin{center}
\includegraphics[scale=0.665]{./imgs/ClusteringProcess.drawio.png}
\caption{{Esquematización de proceso de agrupación y sus etapas.}}
\label{fig:proceso_agrupacion}
\end{center}
\end{figure}

La Figura \ref{fig:proceso_agrupacion} presenta las diversas tareas que intervienen en el proceso de agrupación que se va a desarrollar en el presente componente, el gráfico fue creado en base al cuaderno de trabajo utilizado para realizar clustering, el cual se encuentra disponible en el Anexo \ref{anexo:notebook_clustering}. El diagrama ilustra la secuencia de trabajo que se implementó: obtener las curvas desde la base de datos, ajustar las curvas para comparabilidad, determinar el número óptimo de agrupaciones, optimizar parámetros de los algoritmos, aplicar los algoritmos, generar agrupaciones, evaluar las agrupaciones (tanto cuantitativa como cualitativamente) y seleccionar el algoritmo con el mejor desempeño.
...
\multicolumn{1}{c|}{\textbf{Descripción y propósito}} \\
\hline
\endfirsthead
...
\begin{enumerate}
    ...
    \begin{figure}[H]
    \begin{center}
    \includegraphics[scale=0.68]{./imgs/cm_elbow_method.png}
    ...
    Se realizó un proceso de hiperparametrización para cada uno de los algoritmos de agrupamiento elegidos. Este procedimiento consistió en evaluar diferentes combinaciones de parámetros significativos para cada algoritmo, como los criterios de inicialización, el número máximo de iteraciones y parámetros particulares de cada algoritmo. La Tabla \ref{tab:param_grid_clustering} sintetiza los hiperparámetros más importantes que se tuvieron en cuenta durante el proceso de optimización, los cuales fueron elegidos según su efecto en la calidad de las agrupaciones.
    ...
    \multicolumn{1}{c|}{\textbf{Valores evaluados}} \\
    \hline
    \endfirsthead
...
\subsection*{\underline{Evaluación}}
...
\subsection*{\underline{Despliegue}}
...
\chapter[RESULTADOS, CONCLUSIONES Y RECOMENDACIONES]{%
\bfseries\fontsize{16pt}{16pt}\selectfont RESULTADOS, CONCLUSIONES Y RECOMENDACIONES}
...
\subsection*{\underline{Evaluación cuantitativa de los algoritmos de clustering}}\label{subsec:evaluacion_algoritmos}
...
Respecto a esta métrica, se hace hincapié en la similitud estructural de las curvas obtenidas y no en su demanda absoluta, lo cual resulta especialmente adecuado para el análisis de curvas normalizadas, en las que el interés principal está relacionado con la forma del perfil de consumo \cite{labeeuw2013residential,Chicco2012OverviewClustering}. De este modo, los valores obtenidos evidencian diferencias en la homogeneidad de los grupos generados, constituyendo así un criterio para valorar la calidad de los agrupamientos.

\subsubsection*{Métricas internas complementarias de validación}
De forma complementaria, se utilizaron las métricas internas de validación ampliamente utilizadas en la literatura como el índice de Calinski-Harabasz (CHI), el índice de Davies-Bouldin (DBI) y el Silhouette Score, con la finalidad de estudiar la compacidad y la separación de los clústeres obtenidos \cite{Chicco2012OverviewClustering,li2023shapebased}. La Tabla \ref{tab:metricas_clustering} muestra, de manera cuantitativa, el rendimiento de los algoritmos analizados.
    ...
    \multicolumn{1}{c|}{\textbf{GMM}} &
    \multicolumn{1}{c|}{\textbf{Birch}} &
...
\begin{figure}[H]
    \begin{center}
    \includegraphics[scale=0.305]{./imgs/curvas_agrupadas_mejor_algoritmo2.png}
...
\subsection*{\underline{Selección final del mejor algoritmo}}
...
\section{Archivo docker-compose.yaml}
\label{anexo:docker_compose}

\href{https://github.com/azambrano43/clasificacion-clientes-eeq/blob/03af73c395536e0d1e2f2c7efd2fa13fe131e5a5/docker-compose.yaml}{Disponible en línea}

\section{Código Python del DAG del proceso ETL}
\label{anexo:dag_etl}

\href{https://github.com/azambrano43/clasificacion-clientes-eeq/blob/03af73c395536e0d1e2f2c7efd2fa13fe131e5a5/dags/potencia_aparente_etl_dag.py}{Disponible en línea}

\section{Cuaderno de trabajo de clustering}
\label{anexo:notebook_clustering}

\href{https://github.com/azambrano43/clasificacion-clientes-eeq/blob/03af73c395536e0d1e2f2c7efd2fa13fe131e5a5/notebooks/pruebas_cluster.ipynb}{Disponible en línea}

\section{Reporte final del proyecto de minería de datos}
\label{anexo:reporte_final}

\href{https://github.com/azambrano43/clasificacion-clientes-eeq/blob/03af73c395536e0d1e2f2c7efd2fa13fe131e5a5/outputs/Despliegue%20-%20Reporte%20final.pdf}{Disponible en línea}
